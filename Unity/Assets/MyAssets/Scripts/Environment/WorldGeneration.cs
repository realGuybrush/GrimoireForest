...
public partial class WorldManagement : MonoBehaviour
{
    ...
    {
        GameObject GO = GameObject.Instantiate(environmentFactory.BiomePrefabs[(int)MT.biome1][(int)MT.biome2].TilePrefab, new Vector3(xTileOffset * TileWidth, 0.0f, 0.0f), new Quaternion(), Environment.transform);
        ...
        for (int i = 0; i < 9; i++)
        {
            for (int j = 0; j < GlobalMap.BlocksInTile; j++)
            {
                ...
                Instantiate(environmentFactory.BiomePrefabs[(int)MT.biome1][(int)MT.biome2].BlockPrefabs[MT.blocks[i][j]], new Vector3((j-GlobalMap.BlocksInTile/2)*1.5f+ xTileOffset*(GlobalMap.BlocksInTile*1.5f), (i)*-1.5f,  0.0f), new Quaternion(), GO.transform.GetChild(6));
            }
        }

        for (int i = 0; i < MT.TilePlatforms.Count; i++)
        {
            Instantiate(environmentFactory.BiomePrefabs[(int)MT.biome1][(int)MT.biome2], MT.TilePlatforms[i], Environment.transform.GetChild(0).transform);
        }
    ...
    void SetTileEntities(MapTile MT)
    {
        ...
        for (int i = 0; i < environmentFactory.BiomePrefabs[(int)MT.biome1][(int)MT.biome2].EntitiesPrefabs.Count; i++)
        {
            newAmount = Random.Range(0, environmentFactory.BiomePrefabs[(int)MT.biome1][(int)MT.biome2].EntitiesAmounts[i]);
    ...
    {
        GlobalMap.Generate_Map(environmentFactory.BiomePrefabs[0]);
